...
namespace fractalism {
  namespace gpu {
    namespace opengl {
      ...
      inline static void setUniforms3D(float aspectRatio) {
        glm::dvec3& trackball = App::get<Settings>().trackball;

        glm::vec3 eye = glm::vec3(
          trackball.r * glm::sin(trackball.t) * glm::cos(trackball.p),
          trackball.r * glm::sin(trackball.t) * glm::sin(trackball.p),
          trackball.r * glm::cos(trackball.t));
      ...
      void GLRenderer::render(wxGLCanvas & canvas, GLuint texture) const {
        ...
        case options::Dimensions::three: {
          setUniforms3D(static_cast<float>(width) / static_cast<float>(height));
