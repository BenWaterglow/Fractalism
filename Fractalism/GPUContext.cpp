...
GPUContext::GPUContext(wxGLCanvas& canvas, wxStatusBar& statusBar) : glCtx((statusBar.PushStatusText("Initializing OpenGL context..."), &canvas)) {
  if (!glCtx.IsOK()) {
  ...
  if (err != GL_NO_ERROR) {
    throw GLError("Could not initialize GLEW: %s", glewGetErrorString(err));
  }
  ...
  statusBar.PushStatusText("Initializing OpenCL context...");
  try {
...
cl::Program GPUContext::buildProgram(const char* function, const char* numberSystemDefinitions, size_t maxNumberSystemSize, double escapeValue) const {
  cl::Program program;
  try {
